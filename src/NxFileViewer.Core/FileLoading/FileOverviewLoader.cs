...
public class FileOverviewLoader : IFileOverviewLoader
{
    ...
    private class FileOverviewLoaderInternal(ILogger logger)
    {
        ...
        public FileOverview CreateXciOverview(XciItem xciItem)
        {
            ...
            _logger.LogError(LoadingLocalizationKeys.LoadingErrorXciSecurePartitionNotFoundLog);
            return fileOverview;
        ...
        private IEnumerable<CnmtContainer> BuildCnmtContainers(PartitionFileSystemItemBase partitionItem)
        {
            ...
            if (cnmtItems.Length <= 0) {
                _logger.LogError(LoadingLocalizationKeys.LoadingErrorNoCnmtFoundLog);
            }
            ...
            foreach (var cnmtItem in cnmtItems) {
                ...
                foreach (var cnmtEntryItem in cnmtItem.ChildItems) {
                    ...
                    if (referencedNcaItem == null) {
                        _logger.LogError(LoadingLocalizationKeys.LoadingErrorNcaFileMissingLog.SafeFormat(cnmtEntryItem.NcaId, cnmtEntryItem.NcaContentType));
                        continue;
                    }
                    ...
                    if (cnmtEntryItem.NcaContentType == ContentType.Control) {
                        ...
                        if (nacpItem == null) {
                            _logger.LogError(LoadingLocalizationKeys.LoadingErrorNacpFileMissingLog.SafeFormat(NacpItem.NacpFileName));
                        }
                        else {
                    ...
                    if (programSection == null) {
                        _logger.LogError(LoadingLocalizationKeys.LoadingErrorNcaMissingSectionLog.SafeFormat(cnmtEntryItem.NcaContentType, NcaSectionType.Code));
                    }
                    else {
                        ...
                        if (!programSection.IsSparse) {
                            ...
                            if (mainItem == null) {
                                _logger.LogError(LoadingLocalizationKeys.LoadingErrorMainFileMissingLog.SafeFormat(MainItem.MainFileName));
                            }
                        }
                        else {
        ...
        private byte[]? LoadExpectedIcon(SectionItem sectionItem, NacpLanguage nacpLanguage)
        {
            ...
            if (iconItem == null) {
                _logger.LogError(LoadingLocalizationKeys.LoadingErrorIconMissingLog.SafeFormat(expectedFileName));
                return null;
            }
            ...
            try {
                ...
                _logger.LogError(ex, LoadingLocalizationKeys.LoadingErrorFailedToLoadIconLog.SafeFormat(ex.Message));
                return null;
            }
        }
